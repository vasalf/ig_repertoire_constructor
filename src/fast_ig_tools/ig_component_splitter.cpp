...
public:
    ...
    std::vector<seqan::Dna5String> GetReadDNAs() const {
        std::vector<seqan::Dna5String> res;
        std::transform(begin(), end(),
                       std::back_inserter(res),
                       [](const std::shared_ptr<Read>& r) -> seqan::Dna5String {
                           return r->read;
                       });
        return res;
    }
...
public:
    ...
    virtual std::vector<Component<std::string> > SplitComponent(Component<std::string> &component) override {
        component.Consensus() = consensus(component.GetReadDNAs());
        return { component };
    }
};
...
class LegacySplitterAlgorithm : public SplitterAlgorithm<map_type> {
    ...
    void split_component(Component<std::string> component,
                         std::vector<Component<std::string> > res,
        ...
        std::vector<seqan::Dna5String> reads_in_component = component.GetReadDNAs();
        
        seqan::String<seqan::ProfileChar<seqan::Dna5> > profile;
        ...
        for (const seqan::Dna5String& read : reads_in_component) {
        ...
        for (std::size_t i = 0; i != min_len; i++) {
            std::array<std::pair<std::size_t, std::size_t>, seqan::ValueSize<seqan::Dna5>::VALUE> letters;
            for (std::size_t k = 0; k < seqan::valueSize<seqan::Dna5>(); k++)
        ...
        Component<std::string> majory(component.GetId()), secondary(component.GetId());
        std::vector<std::shared_ptr<Read> > other;
        for (std::shared_ptr<Read>& read : component) {
        ...
        auto majory_consensus = consensus_hamming(majory.GetReadDNAs());
        auto secondary_consensus = consensus_hamming(secondary.GetReadDNAs());

        for (std::shared_ptr<Read>& read : other) {
        ...
        if (this->cfg().discard) {
            for (std::shared_ptr<Read>& read : secondary) {
                Component<std::string> c(secondary.GetId());
                c.push_back(read);
