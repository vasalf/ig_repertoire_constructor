...
namespace vj_finder {
    void VJParallelProcessor::Initialize() {
        ...
        for(size_t i = 0; i < num_threads_; i++)
            info_per_thread[i].csv_stat.open(output_dir_ + "/stat_" + RandomString() + ".csv");
    }
    ...
    std::string VJParallelProcessor::RandomString() {
        std::string ans;
...
#pragma omp parallel for schedule(dynamic)
        for(size_t i = 0; i < read_archive_.size(); i++) {
            ...
            VJQueryProcessor vj_query_processor(algorithm_params_, read_archive_, info_per_thread[thread_id].csv_stat, v_db_, j_db_);
